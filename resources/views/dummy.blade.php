...
$sql = "SELECT id,aadhar, name,address,city,category,phone1,phone2,vist_method,issue_date,submission_date,sanction_status,cheque_issue_date,cheque_status,doc1,doc2 FROM applicants WHERE active_status = 1";
...
echo"<center>";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        {{ csrf_field() }}
        <?php
        echo"<tr onclick='showappl()'><td name='delid'> ".$row["id"]."</td><td>".$row["aadhar"]."</td><td>".$row["name"]."</td><td>".$row["address"]."</td><td>".$row["city"]."</td><td>".$row["category"]."</td><td>".$row["phone1"]."</td><td>".$row["phone2"]."</td><td>".$row["vist_method"]."</td><td>".$row["issue_date"]."</td><td>".$row["submission_date"]."</td><td>".$row["sanction_status"]."</td><td>".$row["cheque_issue_date"]."</td><td>".$row["cheque_status"]."</td><td><input type='submit' formaction='/doc1' name='doc1' value = 'view'></td><td><input type='submit' name='doc2' formaction='/doc2' value = 'view'></td><td><input type='submit' class='click-btn' type='submit' name='delbutton' value='delete'></form></td><td><button class='click-btn' onclick='modalopen()'>EDIT</button></td><td><button class='click-btn' onclick='modalopen_delete()'>CLICK HERE</button></td></tr>";
    }
...
function delete()
{
  if(isset($_POST['delbutton'])){
     ...
     $update_active = Update::where('id',$_POST['delid'])
